...
(function() {
  ...
  window.MapView = (function() {
    ...
    function MapView(collection, center_point) {
      ...
      this.need_rendering = true;
      this.render();
      this.collection.bind('reset', function() {
        _this.render();
        return _this.need_rendering = true;
      });
    }
    ...
    MapView.prototype.setMarkers = function(map, markerBounds) {
      return _.each(this.collection.models, function(business) {
        business.clearMarker();
        if (window.filter.match(business)) {
          return business.setMarker(map, markerBounds);
        }
      });
    };
    ...
    MapView.prototype.reRender = function() {
      console.log('rerendering need=', this.need_rendering);
      if (this.need_rendering === true) {
        this.view = null;
        this.render();
        return this.need_rendering = false;
      }
    };
    ...
    MapView.prototype.render = function() {
      ...
      return this.setMarkers(this.view, this.markerBounds);
    };
  ...
  window.SingleZoomMapView = (function(_super) {

    __extends(SingleZoomMapView, _super);

    function SingleZoomMapView(model, center_point) {
      this.model = model;
      this.center_point = center_point;
      if (!this.model) throw "you need a model to make a MapView";
    }

    SingleZoomMapView.prototype.options = {
      mapTypeId: google.maps.MapTypeId.ROADMAP,
      mapTypeControl: false,
      zoom: 17
    };

    SingleZoomMapView.prototype.render = function() {
      this.model.clearMarker();
      this.prepareMap();
      this.model.setMarker(this.view);
      return this.view.setCenter(this.model.marker.position);
    };

    return SingleZoomMapView;

  })(MapView);

  window.ExtendBoundMapView = (function(_super) {

    __extends(ExtendBoundMapView, _super);

    function ExtendBoundMapView() {
      ExtendBoundMapView.__super__.constructor.apply(this, arguments);
    }

    ExtendBoundMapView.prototype.markerBounds = new google.maps.LatLngBounds();

    ExtendBoundMapView.prototype.markerBoundsZoomOut = 0.1;

    ExtendBoundMapView.prototype.render = function() {
      var extendPoint;
      ExtendBoundMapView.__super__.render.call(this);
...
(function() {
  ...
  window.Event = Backbone.Model.extend({
    startDate: function() {
      return Date.parse(this.get('start')).toString("yyyy-MM-dd");
    },
    startHour: function() {
      return Date.parse(this.get('start')).toString("h:mm");
    },
    startAmPm: function() {
      return Date.parse(this.get('start')).toString("tt");
    },
    business: function() {
      return business_list.get(this.get('business_id'));
    },
    businessName: function() {
      return this.business().get('name');
    },
    businessImageName: function() {
      return this.business().imageName();
    },
    businessIconName: function() {
      return this.business().iconName();
    },
    userFavoritePrefix: function() {
      if (!_.include(Filter.userFavorites, this.get('business_id'))) return 'un';
    }
  });
  ...
  window.Business = Backbone.Model.extend({
    ...
    serviceName: function() {
      if (!_.isNumber(parseInt(this.get('service_type')))) return '';
      return this.service_type_names[this.get('service_type')].toLowerCase();
    },
    imageName: function() {
      return "/assets/" + (this.serviceName()) + ".png";
    },
    iconName: function() {
      return "/assets/" + (this.serviceName()) + "_icon.png";
    },
    ...
    setMarker: function(map, markerBounds) {
    ...
    setMarkerIcon: function(scaleFactor) {
      return this.marker.setIcon(this.makeIcon(scaleFactor));
    },
    makeMarker: function(map) {
    ...
    makeIcon: function(scaleFactor) {
      if (scaleFactor === void 0) scaleFactor = 1;
      return new google.maps.MarkerImage(this.imageName(), null, null, null, new google.maps.Size(30 * scaleFactor, 40 * scaleFactor));
    }
  });
  ...
  window.BusinessList = Backbone.Collection.extend({
    ...
    reset: function(collection) {
      this.each(function(business) {
        return business.clearMarker();
      });
      return Backbone.Collection.prototype.reset.call(this, collection);
    },
    clearSelected: function() {
      if (this.selected_id) this.get(this.selected_id).setMarkerIcon();
      this.selected_id = null;
      if (this.selected_view) this.selected_view.close();
      return this.selected_view = null;
    },
    setSelected: function(business_id, business_view) {
      if (this.selected_id) this.get(this.selected_id).setMarkerIcon();
      this.get(business_id).setMarkerIcon(1.3);
      this.selected_id = business_id;
      return this.selected_view = business_view;
    }
  });
...
(function() {
  ...
  Handlebars.registerHelper("rounded", function(number) {
    if (number !== void 0) return parseFloat(number).toFixed(2);
  });

  Handlebars.registerHelper("hour", function(string) {
    var date;
  ...
  Handlebars.registerHelper("format_ymd_date", function(date) {
    if (date) {
  ...
  Handlebars.registerHelper("phone_format", function(phone) {
  ...
  Handlebars.registerHelper("each_hour", function(array, fn) {
    var buffer, item, num, _ref;
    buffer = "";
    for (num = 0, _ref = array.length - 1; 0 <= _ref ? num <= _ref : num >= _ref; 0 <= _ref ? num++ : num--) {
      item = array[num];
  ...
  Handlebars.registerHelper("header", function(date) {
    var str;
    if (!_.isDate(date)) return;
    str = date.equals(Date.today()) ? "Today" : date.equals(Date.today().addDays(1)) ? "Tommorrow" : date.toString('dddd');
    return str += " " + (date.toString('MMMM d'));
  });
  ...
  window.EventView = Backbone.View.extend({
    ...
    initialize: function(options) {
      ...
      this.template = Handlebars.compile($('#event_template').html());
      return this.render();
    },
    showBusiness: function() {
      var bview;
      if (this.cid === (business_list.selected_view ? business_list.selected_view.event_view.cid : void 0)) {
        return business_list.clearSelected();
      } else {
        business_list.clearSelected();
        bview = new BusinessView({
          event_view: this
        });
        return business_list.setSelected(this.model.business().get('id'), bview);
      }
    },
    render: function() {
      if (this.model) {
        this.elem = $(this.el).html(this.template(this.model));
        this.elem.attr('data-id', this.model.get('id'));
      }
      return this;
    }
  });
  ...
  window.EventDayHeaderView = Backbone.View.extend({
    ...
    render: function() {
      ...
      return this.elem = $(this.el).html(html).attr('data-role', 'list-divider');
    }
  });
  ...
  window.EventListView = Backbone.View.extend({
    initialize: function(event_list) {
      ...
      return this.event_list.bind('reset', function() {
        return _this.render();
      });
    },
    render: function() {
      var date, days_events, events, num;
      this.el.empty();
      events = _(this.event_list.models).groupBy(function(event) {
        return event.startDate();
      });
      for (num = 0; num <= 13; num++) {
        date = Date.today().addDays(num);
        days_events = _.select(events[date.toString("yyyy-MM-dd")], function(event) {
          return window.filter.match(event.business());
        });
        if (!_.any(days_events)) continue;
        this.buildEventsForDay(date, days_events);
      }
      return window.votes.show();
    },
    buildEventsForDay: function(date, events) {
      this.el.append(new EventDayHeaderView({
        'date': date
      }).elem);
      return _.each(events, function(event) {
        ...
        return event_list_view.el.append(event_view.elem);
      });
    }
  });
  ...
  window.BusinessView = Backbone.View.extend({
    ...
    className: 'business',
    initialize: function(options) {
      this.event_view = options.event_view;
      this.template = Handlebars.compile($('#business_info_template').html());
      return this.render();
    },
    render: function() {
      var _this = this;
      this.elem = $(this.el).html(this.template(this.event_view.model.business())).hide();
      this.event_view.elem.after(this.elem);
      this.event_view.elem.find('.description').addClass('open').show();
      return this.elem.slideDown('slow', function() {
        var day_number;
        day_number = _this.event_view.date.getDay();
        return _this.$(".line[data-day_num=" + day_number + "]").addClass('bold');
      });
    },
    close: function() {
      var _this = this;
      return this.elem.slideUp('slow', function() {
        _this.event_view.elem.find('.description').hide();
        _this.event_view.elem.removeClass('open');
  ...
  window.Filter = (function() {
    ...
    Filter.service_type_cafe = true;

    Filter.service_type_restaurant = true;

    Filter.service_type_bar = true;

    Filter.serviceTypes = [0, 1, 2];

    Filter.filtering_favorites = false;

    Filter.userFavorites = [];

    Filter.prototype.setServiceType = function(type, value) {
      var name;
      console.log(type, value);
      Filter[type] = value;
      Filter.serviceTypes = [];
      for (name in service_type_constants) {
        if (Filter[name]) Filter.serviceTypes.push(service_type_constants[name]);
      }
      window.event_list_view.render();
      return window.map_view.need_rendering = true;
    };

    Filter.prototype.setFilteringByFavorites = function(bool) {
      Filter.filtering_favorites = bool;
      window.event_list_view.render();
      return window.map_view.render();
    };

    Filter.prototype.setFavorite = function(business_id) {
      var img, prefix;
      prefix = '';
      if (_.include(Filter.userFavorites, business_id)) {
        $.post('/users/unset_favorite', {
          b: business_id
        });
        ...
        $.post('/users/set_favorite', {
          b: business_id
        });
      }
      img = $("img[data-business_id='" + business_id + "']");
      return img.attr({
        src: "assets/fav_" + prefix + "selected.png"
      });
    };

    Filter.prototype.match = function(business) {
      if (Filter.filtering_favorites) {
        if (!_.include(Filter.userFavorites, business.get('id'))) return false;
      }
      return _.include(Filter.serviceTypes, business.get('service_type'));
    };

    Filter.prototype.setValues = function() {
      return $('.filter input[type=checkbox]').each(function(index, elem) {
        var id;
        id = $(elem).attr('id');
        return $(elem).prop('checked', Filter[id]);
      });
    };
  ...
  window.Votes = (function() {
    ...
    Votes.prototype.votes = [];

    Votes.prototype.votes_hash = {};

    Votes.prototype.setVotes = function(votes) {
      var _this = this;
      this.votes = votes;
      return _.each(votes, function(vote) {
        return _this.votes_hash[vote._id] = vote;
      });
    };

    Votes.prototype.show = function() {
      var _this = this;
      return _.each(this.votes, function(info) {
        return _this.showNumbers(info);
      });
    };

    Votes.prototype.resetVotes = function(data) {
      var _this = this;
      this.votes_hash[data._id] = data;
      _.each(this.votes, function(info, index) {
        if (info._id === data._id) {
          _this.votes[index] = data;
          return _this.showNumbers(data);
        }
      });
      if (window.sorter.sort_type === 'popular') return window.event_list.sort();
    };

    Votes.prototype.showNumbers = function(info) {
      var elem;
      elem = $(".event[data-id='" + info._id + "']");
      elem.find(".vote.up .number").html(info.votes["up_count"] || 0);
      return elem.find(".vote.down .number").html(info.votes["down_count"] || 0);
    };
  ...
  window.Sort = (function() {
    ...
    Sort.prototype.sort_type = 'recent';

    Sort.prototype.sorts = {
      recent: function(event) {
        return event.get('start');
      },
      popular: function(event) {
        return -window.votes.votes_hash[event.id].votes['point'];
      },
      business: function(event) {
        return event.businessName();
      }
    };

    Sort.prototype.setSortType = function(type) {
      this.sort_type = type;
      window.event_list.comparator = this.sorts[type];
      return window.event_list.sort();
    };
...
(function() {
  ...
  window.Events.refresh = function(data) {
    ...
    return $("#location_search").val(data.center.title.replace("\n", ' '));
  };
  ...
  $(document).ready(function() {
    ...
    $('.facebook_login.link').live('click', function(e) {
      return $('.facebook_register').show();
    });
      ...
      if ($('#content.home').data('logged_in')) {
        $("#college_search").autocomplete({
          ...
          select: function(event, ui) {
            ...
            return window.location.href = "/event_list?college=" + ui.item.id;
          }
        });
      } else {
        $("#college_search").autocomplete({
          source: function(req, res) {
            return $.get('/consumers/search_college', {
              term: req.term
            }, function(data) {
          ...
          select: function(event, ui) {
            ...
            return $.ajax({
              ...
              type: 'post',
              data: {
                college: ui.item.label
              },
              success: function(response) {
                return $('.college_found').show();
              },
              error: function(response) {
                return alert('The college selected is not valid');
              }
            });
          }
        });
    ...
    $('#notify_me').live('click', function() {
      var college, email;
      email = $('#notify_email').val();
      college = $('#college_search').val();
      return $.post('/consumers/notify', {
        email: email,
        college: college
      }, function() {
        return $('.notice').html("Done. When the college '" + college + "' is available we will notify you at '" + email + "'");
      });
    });
    $('.link_div .link').live('click', function(event) {
      var direction, elem, type;
      elem = $(event.currentTarget);
      type = elem.data('type');
      direction = elem.data('direction');
      elem.toggleClass('selected_link');
      return $("." + type).toggle('slide', {
        direction: direction
      });
    });
    $('.filter input[name^=service_type]').live('click', function(event) {
      var elem;
      elem = $(event.currentTarget);
      return filter.setServiceType(elem.attr('id'), elem.prop('checked'));
    });
    $('input[name=filtering_favorites]').live('click', function(event) {
      var elem;
      elem = $(event.currentTarget);
      return filter.setFilteringByFavorites(elem.prop('checked'));
    });
    $('.images img[rel=favorite]').live('click', function(event) {
      var elem;
      elem = $(event.currentTarget);
      return filter.setFavorite(elem.data('business_id'));
    });
    $('.images img[rel=vote]').live('click', function(event) {
      var elem, event_id, vote;
      elem = $(event.currentTarget);
      event_id = elem.data('id');
      vote = elem.data('vote');
      return $.post('/users/event_vote', {
        event: event_id,
        vote: vote
      }, function(data) {
        if (data) return window.votes.resetVotes(data);
      });
    });
    $('.sort_div .link').live('click', function(event) {
      var elem, type;
      elem = $(event.currentTarget);
      type = elem.data('type');
      $('.sort_div .link').removeClass('selected_link');
      elem.addClass('selected_link');
      return window.sorter.setSortType(type);
    });
    return $('.event .info, .description').live('hover', function() {
      return $(this).closest('.event').toggleClass('hover');
    });
  });
...
(function() {
  ...
  $(document).ready(function() {
    ...
    if (data && data.events) window.Events.refresh(data);
    return $('ul').listview('refresh');
  });

  $('[data-role=page]').live('pageshow', function(event) {
    if (event.target.id === 'map_page') {
      if (window.map_view) return window.map_view.reRender();
    }
  });

  $('label[for^=checkbox]').live('click', function(event) {
    var elem;
    elem = $(event.currentTarget);
    debugger;
    console.log(elem.data('service_type'), elem.prev('input').prop('checked'));
    return filter.setServiceType(elem.data('service_type'), elem.prev('input').prop('checked'));
  });
