class App.Model.Event extends Backbone.Model
  ...
  business: => window.event_list_container_view.business_list.get( @get('business_id') )
  businessName: -> @business().get('name')
  businessImageName: -> @business().imageName()
  businessIconName: -> @business().iconName()
  userFavoritePrefix: -> 'un' unless _.include( window.event_list_container_view.filter.get('user_favorites'), @get('business_id') )
